...
<body>
	...
	<div style="text-align: center;color:red;font-size:25px">公海客户页面</div>
	<script type="text/html" id="customerlist_toolbar">
		<div>
			...
			<button class="layui-btn layui-btn-sm" onclick="documentary()">跟单</button>
		</div>
	</script>
	<script type="text/html" id="operate_toolbar">
		<div>
			<button class="layui-btn layui-btn-xs layui-btn-radius" lay-event="details">详情</button>
  			...
  			<button class="layui-btn layui-btn-xs layui-btn-radius" lay-event="documentary">跟单</button>
		</div>
	</script>
	<table class="layui-table"
		lay-data="{url:'sys/showCustomer_public',toolbar:'#customerlist_toolbar',id:'customerlist',page:true}" lay-filter='customertable'>
		<thead>
			<tr>
	...
	<script type="text/javascript">
		...
		table.on('tool(customertable)',function(obj){
			...
			}else if(layEvent === 'documentary'){
				layer.confirm('确认要对'+checkdata.customer_name+'进行跟单操作吗?',{
					icon:3,
					title:'提示'
				},function(index){
					$.post("sys/documentaryCustomer",{
						"customer_id":checkid,
	...
	function documentary(){
		var checkdata = table.checkStatus("customerlist").data;
		if(checkdata.length > 0){
			layer.confirm('确认要对选中的账号进行跟单操作吗?',{
				icon:3,
				title:'提示'
			},function(index){
				for(var i = 0;i<checkdata.length;i++){
